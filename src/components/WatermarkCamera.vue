...
async function onTakePhoto(facingMode: 'user' | 'environment') {
  try {
    ...
    const locationData = location.status === 'fulfilled' ? location.value : { latitude: 0, longitude: 0, address: '位置信息获取失败了，请申请高德地图key，再配置amapKey,啊哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈' }
    const timeData = time.status === 'fulfilled' ? time.value : getLocalTime()
    
    // 生成水印
    const watermarkedPath = await drawWatermark(photoPath, {
      address: locationData.address || '位置信息获取失败了，请申请高德地图key，再配置amapKey，啊哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈',
      time: timeData,
...
async function uploadImage(filePath: string): Promise<any> {
  try {
    ...
    if (uploadConfig.customUploadFn) {
      const file = base64ToFile(filePath, 'photo.jpg')
      result = await uploadConfig.customUploadFn(file)
    } else {
      const file = base64ToFile(filePath, 'photo.jpg')
      result = await uploadFile(file, action)
    }
